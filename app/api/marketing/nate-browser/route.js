...
export async function GET(req){
 ...
 const key=process.env.BRIGHT_DATA_API_KEY||'';let password='',ws=null,bytes=0;
 ...
 try{
  ...
  if(op==='status')return Response.json({pages:jobs.map(j=>({page:j.request.page,status:j.status,posts:j.diagnostics.postCount,oldest:j.diagnostics.oldest}))},{headers:H});
  if(!['setup','run'].includes(op))throw Error('Unsupported operation');
  ...
  const zones=await bd('/zone/get_active_zones');let created=false;
  if(!zones.some(z=>z.name===ZONE&&z.type==='browser_api')){await bd('/zone',{zone:{name:ZONE,type:'browser_api'},plan:{type:'browser_api'}});created=true}
  if(op==='setup')return Response.json({configured:true,zone:ZONE,created},{headers:H});
  ...
  const send=(method,params={},sessionId)=>new Promise((resolve,reject)=>{const id=++seq,timer=setTimeout(()=>{pending.delete(id);reject(Error('Browser command timeout: '+method))},85000);pending.set(id,{resolve,reject,timer});ws.send(JSON.stringify({id,method,params,...(sessionId?{sessionId}:{})}))});
  ...
  await cmd('Network.setBlockedURLs',{urls:['*://pbs.twimg.com/*','*://video.twimg.com/*','*://dd133.com/*','*://llvpn.com/*','*://www.googletagmanager.com/*','*://*.doubleclick.net/*']});
  await cmd('Page.navigate',{url:PROFILE});
  let ready=false;for(let n=0;n<24;n++){const r=await cmd('Runtime.evaluate',{expression:'Boolean(document.querySelector(".add-nw-event[data-query=\\"1544391592106201091\\"]"))',returnByValue:true});if(r.result?.value){ready=true;break}await sleep(2000)}
  if(!ready){const r=await cmd('Runtime.evaluate',{expression:'({title:document.title,body:document.body?.innerText?.slice(0,1000)})',returnByValue:true});throw Error('Public profile not ready: '+JSON.stringify(r.result?.value))}
  const results=[],end=Date.now()+170000;
  while(page<=400&&results.length<50&&Date.now()<end&&bytes<25000000){
   ...
   try{
    ...
    results.push({id,page,posts:entries.length,authored:authored.length,oldest:diagnostics.oldest,hasMore});if(!hasMore)break;cursor=next;page++;await sleep(300);
   }catch(e){await store('patch',{id,patch:{status:'failed',record_count:1,error:{message:safe(e)}}});throw e}
  }
  return Response.json({results,sessionBytes:bytes,completeHistoryVerified:false},{headers:H});
 }catch(e){return Response.json({error:safe(e)},{status:502,headers:H})}finally{if(ws)try{ws.close()}catch{}}
}
