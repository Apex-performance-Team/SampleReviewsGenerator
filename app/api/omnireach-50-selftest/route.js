...
export async function GET(req){
  ...
  const scanFailure=!scanRes.ok||!scan?.referenceSet?{status:scanRes.status,error:scan?.error||'referenceSet missing',diagnostics:scan?.diagnostics||null,brightData:scan?.brightData||null}:null;
  ...
  let current=scanFailure?knownReferenceSet(scanFailure):scan.referenceSet,passes=[];
