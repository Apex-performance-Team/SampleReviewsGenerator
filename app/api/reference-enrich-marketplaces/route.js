...
export async function POST(req){
  ...
  for(const {src,result} of results){
    ...
    for(const r of result.reviews||[]){const bodyText=clean(r.body),k=bodyText.toLowerCase().replace(/[^a-z0-9]+/g,' ').trim();if(!k||existing.has(k))continue;existing.add(k);appended.push({referenceId:`REF-${hash(`${sourceUrl}|${bodyText}`).slice(0,8)}`,platform,provider,sourceUrl,sourceRating:r.rating??null,sourceTitle:clean(r.title).slice(0,220),sourceBody:bodyText,wordCount:bodyText.split(/\s+/).filter(Boolean).length,sentenceCount:sent(bodyText)});added++}
    ...
    diagnostics.push({sourceUrl,platform,provider,attempted:result.attempted||0,blocked:result.blocked||0,failed:result.failed||0,pending:Boolean(result.pending),snapshotId:result.snapshotId||null,extracted:added,error:result.error||''});
  }
