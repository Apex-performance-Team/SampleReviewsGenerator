...
export async function POST(req){
  ...
  let baseRes;try{baseRes=await v11POST(forwarded)}catch(e){return Response.json({error:e?.message||String(e)},{status:400})}
  let base;try{base=await baseRes.clone().json()}catch{return baseRes}
  if(!baseRes.ok)return baseRes;
  const rs=base?.referenceSet;if(!rs)return baseRes;
  if((rs.sourceCounts||[]).length)return baseRes;

  const discoveryInput={...rs,productDescription:body.productDescription||'',productTitle:body.productTitle||rs.productTitle||'',productUrl:body.productUrl||rs.productUrl||''};
  ...
  if(!candidates.length){
    return Response.json({referenceSet:{...rs,productDescription:body.productDescription||'',amazonFallbackDiscovery:{status:'no_verified_candidate',queries:discovery.queries||[],diagnostics:discovery.diagnostics||{}}}},{headers:{'cache-control':'no-store'}})
  }
  ...
  const out={...rs,productDescription:body.productDescription||'',sourceCounts:rows,aggregateOnlySources:aggregate,confidence:rows.some(x=>x.confidence==='high')?'high':'medium',matchedPages:rows.length,verifiedSourceLinks:rows.length,amazonFallbackDiscovery:{status:'verified',queries:discovery.queries||[],diagnostics:discovery.diagnostics||{},candidates:candidates.map(c=>({asin:c.asin,url:c.url,title:c.title,ratingCount:c.ratingCount,verificationConfidence:c.verificationConfidence,verificationRelation:c.verificationRelation,verificationReason:c.verificationReason}))},provenance:{...(rs.provenance||{}),amazonFallbackUsed:true,amazonFallbackCandidates:rows.length}};
