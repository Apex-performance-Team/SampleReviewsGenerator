...
async function amazonFallback(req,body,rs,baseError){
  ...
  const candidates=discovery.candidates||[],baseRs=rs||emptyReferenceSet(body,baseError);
  if(!candidates.length)return Response.json({referenceSet:{...baseRs,productDescription:discoveryInput.productDescription,amazonFallbackDiscovery:{status:'no_verified_candidate',baseLensError:baseError||null,queries:discovery.queries||[],identity:discovery.identity||null,diagnostics:discovery.diagnostics||{}}}},{headers:{'cache-control':'no-store'}});
