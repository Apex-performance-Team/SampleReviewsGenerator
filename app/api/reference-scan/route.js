...
const DISCOVERY_MODEL='openai/gpt-5.6-sol';
...
function searchSources(j){const out=[];const add=x=>{const u=publicUrl(x);if(u&&!out.includes(u.href))out.push(u.href)};walk(j,o=>{if(o?.source_website_url)add(o.source_website_url);if(o?.url_citation?.url)add(o.url_citation.url);if(o?.type==='url_citation'&&o?.url)add(o.url);if(o?.type==='web_search_result'&&o?.url)add(o.url);if(o?.type==='web_search_call')for(const s of o?.action?.sources||[])add(s?.url)});return out}
function imageSearchResults(j,origin){const out=[],seen=new Set();walk(j,r=>{if(r?.type!=='image_result')return;const source=publicUrl(r.source_website_url),image=publicUrl(r.image_url),thumb=publicUrl(r.thumbnail_url);if(!source||(!image&&!thumb))return;const key=`${canonical(source.href)}|${canonical((image||thumb).href)}`;if(seen.has(key))return;seen.add(key);out.push({sourceUrl:source.href,imageUrl:image?.href||thumb?.href||null,thumbnailUrl:thumb?.href||null,caption:cleanText(r.caption||'').slice(0,240),origin})});return out}
...
async function openaiImageSearchPass(req,{productTitle,productDescription,heroImage,sourceUrl,amazonOnly}){
  ...
  const prompt=`You are doing image-first exact-product discovery for INTERNAL QA MODELING. Use the supplied product image as the primary identity signal. ${scope}\n\nYou MUST call the web-search tool. Use IMAGE WEB SEARCH, not just text search. Search for product photos that visually match the supplied image. Compare silhouette, geometry, component placement, materials, colors, surface details, ports/buttons, packaging, included accessories, printed text/logos/model numbers, and distinctive construction. Private-label versions count when the physical product is the same. Reject merely similar products. Exclude the source store itself. Never quote review bodies.\n\nSOURCE URL: ${sourceUrl}\nPRODUCT TITLE: ${productTitle}\nOFFICIAL PRODUCT CONTEXT:\n${productDescription}\n\nReturn ONLY JSON: {"visualSignals":["specific visible traits"],"queries":["searches you used"],"candidates":[{"url":"https://...","matchConfidence":0.0,"matchReason":"visual evidence","reviewCountEstimate":null,"ratingEstimate":null}]}. Prefer source product-page URLs behind matching images. Do not invent URLs or counts.`;
  const tool={type:'web_search',search_content_types:['image','text'],search_context_size:'high',image_settings:{max_results:amazonOnly?14:12,caption:true}};
  if(amazonOnly)tool.filters={allowed_domains:['amazon.com']};
  const body={model:DISCOVERY_MODEL,reasoning:{effort:'low'},input:[{type:'message',role:'user',content:[{type:'input_text',text:prompt},{type:'input_image',image_url:heroImage.dataUrl,detail:'high'}]}],tools:[tool],tool_choice:'required',include:['web_search_call.results','web_search_call.action.sources'],store:false};
  ...
  const images=imageSearchResults(j,amazonOnly?'amazon_image_search':'broad_image_search'),sources=searchSources(j),calls=(j?.output||[]).filter(x=>x?.type==='web_search_call').length;
  return{visualSignals:Array.isArray(parsed.visualSignals)?parsed.visualSignals.slice(0,12):[],queries:Array.isArray(parsed.queries)?parsed.queries.slice(0,12):[],candidates:Array.isArray(parsed.candidates)?parsed.candidates:[],sources,imageResults:images,diagnostics:{mode:amazonOnly?'amazon':'broad',webSearchCalls:calls,imageResults:images.length,sources:sources.length}}
}
...
async function geminiFallbackDiscover(req,{productTitle,productDescription,heroImage,sourceUrl}){
  ...
  const sources=searchSources(j);return{visualSignals:Array.isArray(parsed.visualSignals)?parsed.visualSignals.slice(0,12):[],queries:Array.isArray(parsed.queries)?parsed.queries.slice(0,12):[],candidates:Array.isArray(parsed.candidates)?parsed.candidates:[],sources,imageResults:[],fallbackUsed:true,diagnostics:{fallback:'gemini',sources:sources.length}}
}
...
async function aiDiscover(req,args){
  try{
    ...
    if(!candidates.length&&!sources.length){const fallback=await geminiFallbackDiscover(req,args);return{...fallback,diagnostics:{...fallback.diagnostics,amazon:amazon.diagnostics,broad:broad.diagnostics,imageSearchEmpty:true}}}
    return{visualSignals:[...new Set([...amazon.visualSignals,...broad.visualSignals])].slice(0,16),queries:[...new Set([...amazon.queries,...broad.queries])].slice(0,20),candidates,sources,imageResults,visualVerifiedCount:verified.length,fallbackUsed:false,diagnostics:{amazon:amazon.diagnostics,broad:broad.diagnostics,visualVerified:verified.length}}
  }catch(e){const fallback=await geminiFallbackDiscover(req,args);return{...fallback,discoveryError:e?.message||String(e),diagnostics:{...fallback.diagnostics,gptImageSearchError:e?.message||String(e)}}}
}
...
async function fillMissingCounts(req,{productTitle,productDescription,pages}){
  ...
  try{
    const r=await fetch(GATEWAY_URL,{method:'POST',headers:{'content-type':'application/json',authorization:`Bearer ${token}`},body:JSON.stringify({model:COUNT_MODEL,input:prompt,tools:[{type:'web_search',search_context_size:'high'}],tool_choice:'required',store:false}),signal:AbortSignal.timeout(70000)});
    const raw=await r.text();let j;try{j=JSON.parse(raw)}catch{j=null}if(!r.ok)return;let parsed={};try{parsed=parseLooseObject(extractText(j))}catch{return}
