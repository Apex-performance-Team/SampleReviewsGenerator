...
async function aiDiscover(req,{productTitle,productDescription,heroImage,sourceUrl}){
  ...
  const prompt=`You are an image-first exact-product discovery agent for INTERNAL QA MODELING. Treat the supplied FIRST/HERO PRODUCT IMAGE as the primary identity signal. The image bytes were downloaded and verified by our server, so you must use the image itself rather than relying on its URL or filename.\n\nAMAZON IS THE FIRST-PRIORITY SOURCE. Begin by searching Amazon for the exact same physical product/private-label equivalent. Run Amazon-focused searches first (including site:amazon.com where useful), identify ASIN-bearing product pages when possible, and exhaust plausible Amazon matches before searching AliExpress, Walmart, eBay, Temu, Etsy, manufacturer/reseller pages, or other stores. If an exact Amazon match exists, rank it ahead of non-Amazon matches.\n\nThe task is closer to reverse-image product search than generic semantic product search. Prioritize visual identity first: silhouette, geometry, component placement, materials, colors, surface details, ports/buttons, packaging, included accessories, printed text/logos/model numbers, and distinctive construction. Use the official product title/context only as supporting evidence. Do NOT accept a merely similar product because its category or keywords match. Exclude the source store itself. Do not quote or reproduce review text.\n\nFor each candidate, if public indexed information exposes the TOTAL review/rating count for that exact listing, include it. Do not invent a count; use null when unavailable.\nSOURCE URL: ${sourceUrl}\nPRODUCT TITLE: ${productTitle}\nOFFICIAL PRODUCT CONTEXT:\n${productDescription}\nReturn ONLY one JSON object: {"visualSignals":["specific things actually visible in the supplied image"],"queries":["..."],"candidates":[{"url":"https://...","matchConfidence":0.0,"matchReason":"specific visual evidence","reviewCountEstimate":null,"ratingEstimate":null}]}. Return at most 20 candidates. Give high matchConfidence only when the listing appears to show the same physical product, not merely the same product type.`;
...
export async function POST(req){try{
  ...
  const candidates=[...map.values()].map(x=>({...x,origins:[...x.origins]})).sort((a,b)=>(Number(isAmazonUrl(b.url))-Number(isAmazonUrl(a.url)))||(b.aiConfidence-a.aiConfidence)).slice(0,18),pages=[];let cursor=0;
  async function worker(){while(true){const i=cursor++;if(i>=candidates.length)return;const c=candidates[i];
    if(isAmazonUrl(c.url)){
      const asin=amazonAsin(c.url),directSourceUrl=asin?amazonCanonical(c.url):c.url,linkUrl=asin?amazonSearch(c.url):listingSearch(c.url,'Amazon',productTitle),score=Math.max(c.aiConfidence,c.discoveryReviewCount!=null?.5:0);
    ...
    try{
      ...
      const reviews=ing.reviews,reviewBoost=reviews.length?0.08:0,matchScore=Math.min(1,c.aiConfidence*.58+titleOverlap*.34+reviewBoost),publicReviewCount=visibleCount??c.discoveryReviewCount,directSourceUrl=p.url,linkUrl=listingSearch(p.url,title,productTitle);
      ...
      const score=Math.max(c.aiConfidence,c.discoveryReviewCount!=null?.5:0),directSourceUrl=c.searchVerified?c.url:null,linkUrl=directSourceUrl?listingSearch(directSourceUrl,null,productTitle):null;
      pages[i]={...c,title:null,status:String(e.message).includes('blocked')?'blocked':'unavailable',matchScore:Number(score.toFixed(3)),confidence:confidence(score),reviewCount:0,reviews:[],publicReviewCount:c.discoveryReviewCount,countKind:c.discoveryReviewCount!=null?'web_estimate':null,ratingEstimate:c.ratingEstimate,error:e.message,directSourceUrl,linkUrl,linkVerified:Boolean(linkUrl),linkVerification:linkUrl?'google_locator_for_web_search_result':'unverified_model_candidate',ingestion:{mode:'unavailable',providers:[],embedded:0,paginationPagesFetched:0,providerEndpointsFetched:0,blocked:String(e.message).includes('blocked')?1:0,failed:String(e.message).includes('blocked')?0:1}}
    }
  }}
