...
async function fetchVisionImage(url,timeout=18000){
  const u=publicUrl(url);if(!u)throw Error('Hero image URL is unsafe or invalid.');
  ...
  if(!r.ok)throw Error(`Hero image returned HTTP ${r.status}.`);
  const contentType=(r.headers.get('content-type')||'').split(';')[0].trim().toLowerCase();
  const bytes=Buffer.from(await r.arrayBuffer());
  if(!contentType.startsWith('image/'))throw Error(`Hero image URL returned ${contentType||'non-image content'}.`);
  if(contentType==='image/svg+xml')throw Error('Hero image is SVG; a raster product image is required for vision matching.');
  if(bytes.length<500)throw Error('Hero image payload is unexpectedly small.');
  if(bytes.length>5*1024*1024)throw Error(`Hero image is ${(bytes.length/1024/1024).toFixed(1)} MB; vision input is capped at 5 MB.`);
...
function searchSources(j){const out=[];const add=x=>{const u=publicUrl(x);if(u&&!out.includes(u.href))out.push(u.href)};for(const o of j?.output||[]){if(o?.type==='web_search_call')for(const s of o?.action?.sources||[])add(s?.url);for(const c of o?.content||[])for(const a of c?.annotations||[])add(a?.url||a?.url_citation?.url)}return out}
...
function buildReferenceRecords(pages){const out=[],seen=new Set();for(const p of pages){if(!p?.reviews?.length)continue;const provider=(p.ingestion?.providers||[]).find(x=>x&&x!=='amazon')||null,platform=host(p.url)||'unknown';for(const r of p.reviews){const body=cleanText(r.body).slice(0,1400);if(body.length<10)continue;const key=body.toLowerCase().replace(/[^a-z0-9]+/g,' ').trim();if(!key||seen.has(key))continue;seen.add(key);out.push({referenceId:`REF-${hash(key)}`,platform,provider,sourceUrl:p.linkUrl||p.url,sourceRating:Number(r.rating)||null,sourceTitle:cleanText(r.title||'').slice(0,180),sourceBody:body,wordCount:body.split(/\s+/).filter(Boolean).length,sentenceCount:sentenceCount(body)});if(out.length>=1200)return out}}return out}
...
function sourceCounts(pages){return pages.filter(p=>p&&p.matchScore>=.4).map(p=>({platform:host(p.linkUrl||p.url)||'unknown',provider:(p.ingestion?.providers||[])[0]||(/amazon\./i.test(host(p.url))?'amazon':null),sourceUrl:p.linkVerified?p.linkUrl:null,title:p.title||null,status:p.status||'found',matchConfidence:Number(p.matchScore.toFixed(3)),confidence:p.confidence||confidence(p.matchScore),publicReviewCount:p.publicReviewCount??null,countKind:p.countKind||null,extractedReviewCount:0,individualExtractedCount:Number(p.reviewCount)||0,pageCount:1,aggregateOnly:Boolean(isAmazonUrl(p.url)),ratingEstimate:p.ratingEstimate??null,error:p.error||null,linkVerified:Boolean(p.linkVerified),linkVerification:p.linkVerification||null})).sort((a,b)=>(Number(b.publicReviewCount)||Number(b.extractedReviewCount)||0)-(Number(a.publicReviewCount)||Number(a.extractedReviewCount)||0))}
...
export async function POST(req){try{
  ...
  const discovery=await aiDiscover(req,{productTitle,productDescription,heroImage,sourceUrl:source.url}),sourceHost=host(source.url),map=new Map(),searchVerified=new Set(discovery.sources.map(canonical));
  const add=(url,confidenceValue=.35,reason='web-search source',meta={},origin='model')=>{const u=publicUrl(url);if(!u||host(u.href)===sourceHost)return;const k=canonical(u.href),old=map.get(k)||{url:k,aiConfidence:0,matchReason:'',discoveryReviewCount:null,ratingEstimate:null,searchVerified:false,origins:new Set()};old.aiConfidence=Math.max(old.aiConfidence,Math.max(0,Math.min(1,Number(confidenceValue)||0)));if(reason)old.matchReason=cleanText(reason).slice(0,220);const rc=countNum(meta.reviewCountEstimate);if(rc!=null)old.discoveryReviewCount=old.discoveryReviewCount==null?rc:Math.max(old.discoveryReviewCount,rc);const rt=num(meta.ratingEstimate);if(rt!=null&&rt<=5)old.ratingEstimate=rt;old.searchVerified=old.searchVerified||origin==='web_search'||searchVerified.has(k);old.origins.add(origin);map.set(k,old)};
  for(const c of discovery.candidates)add(c?.url,c?.matchConfidence,c?.matchReason,c||{},'model');for(const u of discovery.sources)add(u,.35,'AI web-search result',{},'web_search');
  const candidates=[...map.values()].map(x=>({...x,origins:[...x.origins]})).sort((a,b)=>b.aiConfidence-a.aiConfidence).slice(0,18),pages=[];let cursor=0;
  async function worker(){while(true){const i=cursor++;if(i>=candidates.length)return;const c=candidates[i];
    if(isAmazonUrl(c.url)){
      const asin=amazonAsin(c.url),linkUrl=c.searchVerified&&asin?amazonCanonical(c.url):null,score=Math.max(c.aiConfidence,c.discoveryReviewCount!=null?.5:0);
      pages[i]={...c,url:c.url,linkUrl,linkVerified:Boolean(linkUrl),linkVerification:linkUrl?'web_search_plus_canonical_asin':'unverified_model_candidate',title:'Amazon',status:'aggregate_only',matchScore:score,confidence:confidence(score),reviewCount:0,reviews:[],publicReviewCount:c.discoveryReviewCount,countKind:c.discoveryReviewCount!=null?'web_estimate':null,ratingEstimate:c.ratingEstimate,asin,ingestion:{mode:'amazon_aggregate_only',providers:['amazon'],embedded:0,paginationPagesFetched:0,providerEndpointsFetched:0,blocked:0,failed:0}};continue
    }
    try{
      ...
      pages[i]={...c,url:p.url,linkUrl:p.url,linkVerified:true,linkVerification:'server_fetch_final_redirect',title,status:'found',titleOverlap:Number(titleOverlap.toFixed(3)),matchScore:Number(matchScore.toFixed(3)),confidence:confidence(matchScore),reviewCount:reviews.length,reviews,publicReviewCount,countKind:visibleCount!=null?'page':c.discoveryReviewCount!=null?'web_estimate':null,ratingEstimate:c.ratingEstimate,ingestion:{mode:'provider_aware',...ing.diagnostics}}
    }catch(e){
      const score=Math.max(c.aiConfidence,c.discoveryReviewCount!=null?.5:0),verifiedUrl=c.searchVerified?c.url:null;
      pages[i]={...c,title:null,status:String(e.message).includes('blocked')?'blocked':'unavailable',matchScore:Number(score.toFixed(3)),confidence:confidence(score),reviewCount:0,reviews:[],publicReviewCount:c.discoveryReviewCount,countKind:c.discoveryReviewCount!=null?'web_estimate':null,ratingEstimate:c.ratingEstimate,error:e.message,linkUrl:verifiedUrl,linkVerified:Boolean(verifiedUrl),linkVerification:verifiedUrl?'web_search_result':'unverified_model_candidate',ingestion:{mode:'unavailable',providers:[],embedded:0,paginationPagesFetched:0,providerEndpointsFetched:0,blocked:String(e.message).includes('blocked')?1:0,failed:String(e.message).includes('blocked')?0:1}}
    }
  }}
  await Promise.all(Array.from({length:Math.min(5,candidates.length)},worker));
  const accepted=pages.filter(Boolean).filter(x=>x.matchScore>=.4&&(x.linkVerified||x.reviewCount>0||x.publicReviewCount!=null));await fillMissingCounts(req,{productTitle,productDescription,pages:accepted});
  const refs=buildReferenceRecords(accepted),counts=platformCounts(refs),sources=sourceCounts(accepted),amazonAggregate=sources.filter(x=>x.aggregateOnly).map(x=>({platform:x.platform,sourceUrl:x.sourceUrl,asin:x.sourceUrl?amazonAsin(x.sourceUrl):null,sameProductConfidence:x.matchConfidence,reviewCountEstimate:x.publicReviewCount,ratingEstimate:x.ratingEstimate}));
  const referencePages=accepted.filter(x=>x.reviewCount>0||x.publicReviewCount!=null||x.linkVerified),conf=referencePages.some(x=>x.confidence==='high')?'high':referencePages.some(x=>x.confidence==='medium')?'medium':referencePages.length?'low':'none';
  ...
  return Response.json({referenceSet:{version:'individual-reference-v4',provider:'vercel_ai_gateway_gemini_inline_hero_verified_links_plus_provider_aware_ingestion',discoveryModel:DISCOVERY_MODEL,productUrl:source.url,productTitle,totalIndividualReviews:refs.length,availableForGeneration:Math.min(250,refs.length),platformCounts:counts,sourceCounts:sources,aggregateOnlySources:amazonAggregate,references:refs.slice(0,250),confidence:conf,matchedPages:referencePages.length,candidatePages:map.size,verifiedSourceLinks:sources.filter(x=>x.linkVerified).length,ingestion:{embedded:ingestion.embedded,paginationPagesFetched:ingestion.paginationPagesFetched,providerEndpointsFetched:ingestion.providerEndpointsFetched,blocked:ingestion.blocked,failed:ingestion.failed,providers:[...ingestion.providers]},provenance:{imagesScanned:1,heroImageUrl:heroImage.originalUrl,heroImageFinalUrl:heroImage.finalUrl,heroImageBytes:heroImage.bytes,heroImageContentType:heroImage.contentType,imageTransport:'server_downloaded_inline_base64',visualSignals:discovery.visualSignals,searchQueries:discovery.queries,searchSources:discovery.sources.length,generatedAt:new Date().toISOString()},syntheticUseOnly:true,sourceReviewTextExported:false}})
