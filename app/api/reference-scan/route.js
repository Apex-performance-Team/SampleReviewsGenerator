...
async function aiDiscover(req,{productTitle,productDescription,images,sourceUrl}){
  ...
  const prompt=`You are an external product-reference discovery agent for INTERNAL QA MODELING. Analyze the supplied product images plus the official product title/context, then use web search to find ecommerce product pages that are highly likely to describe the same exact product or a visually identical/private-label version. Prioritize sources with customer reviews: Amazon, AliExpress, Walmart, eBay, Temu, Etsy, manufacturer/reseller pages, and stores using Judge.me/Yotpo/Loox/Stamped when relevant. Use visible logos, model numbers, packaging text, shape, accessories, colors, distinctive construction, and the official title/context as search clues. Exclude the source store itself. Do not quote or reproduce review text.\n\nFor each candidate, if public indexed information exposes the TOTAL review/rating count for that exact listing, include it. Do not invent a count; use null when unavailable.\nSOURCE URL: ${sourceUrl}\nPRODUCT TITLE: ${productTitle}\nOFFICIAL PRODUCT CONTEXT:\n${productDescription}\nReturn ONLY one JSON object: {"visualSignals":["..."],"queries":["..."],"candidates":[{"url":"https://...","matchConfidence":0.0,"matchReason":"short reason","reviewCountEstimate":null,"ratingEstimate":null}]}. Return at most 20 candidates.`;
  const content=[{type:'input_text',text:prompt},...images.slice(0,3).map(image_url=>({type:'input_image',image_url,detail:'low'}))];
  const r=await fetch(GATEWAY_URL,{method:'POST',headers:{'content-type':'application/json',authorization:`Bearer ${token}`},body:JSON.stringify({model:DISCOVERY_MODEL,input:[{type:'message',role:'user',content}],tools:[{type:'web_search',search_context_size:'high'}],tool_choice:'auto',include:['web_search_call.action.sources'],store:false,providerOptions:{gateway:{only:['openai']}}}),signal:AbortSignal.timeout(80000)});
...
async function fillMissingCounts(req,{productTitle,productDescription,pages}){
  ...
  try{
    const r=await fetch(GATEWAY_URL,{method:'POST',headers:{'content-type':'application/json',authorization:`Bearer ${token}`},body:JSON.stringify({model:DISCOVERY_MODEL,input:prompt,tools:[{type:'web_search',search_context_size:'high'}],tool_choice:'auto',store:false,providerOptions:{gateway:{only:['openai']}}}),signal:AbortSignal.timeout(70000)});
