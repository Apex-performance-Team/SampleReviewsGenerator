...
export async function GET(req){
  ...
  try{
    ...
    const {r:enrichRes,j:enrichJson}=await postJson(`${origin}/api/reference-enrich-marketplaces`,{referenceSet:baseline},240000);
    ...
    const before=summarize(baseline),after=summarize(enriched),pass=after.totalReferences>=200&&after.totalReferences>=before.totalReferences&&after.amazonReferences>=150;
    return Response.json({ok:pass,stage:pass?'pass':'marketplace_reviews_missing',pdp:{url:scanJson.productUrl,title:scanJson.productTitle,existingReviewCount:scanJson?.extracted?.existingReviewCount??null},baseline:before,enriched:after,checks:{corpusNeverShrank:after.totalReferences>=before.totalReferences,totalAtLeast200:after.totalReferences>=200,amazonAtLeast150:after.amazonReferences>=150,marketplaceBodiesExtracted:after.marketplaceReferences>0,amazonBodiesExtracted:after.amazonReferences>0,ebayBodiesExtracted:after.ebayReferences>0},elapsedMs:Date.now()-started},{status:pass?200:500,headers:{'cache-control':'no-store'}})
