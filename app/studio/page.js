...
export default function StudioPage(){
  ...
  async function scanStore(){
    ...
    try{
      ...
      const rows=j.products.map(p=>({...p,enabled:true,status:'queued'}));setProducts(rows);setMeta({...j,scanned:0,failed:0});
  ...
  function useProduct(p){if(p.status!=='done')return;setF(x=>({...x,productUrl:p.url,productTitle:p.productTitle,productDescription:p.productDescription}));form.current?.scrollIntoView({behavior:'smooth'})}
  ...
  async function processServerRun(id,label,total,{base=0,grandTotal=total}={}){
    let run=await fetchRun(id);
    setActiveRun(run);setProgress({done:Math.min(grandTotal,base+(run.completed_count||0)),total:grandTotal,status:`${label} · ${run.progress_message||statusLine(run)}`});
    for(let i=0;i<140;i++){
      ...
      run=j.run;setActiveRun(run);
      setProgress({done:Math.min(grandTotal,base+(run.completed_count||0)),total:grandTotal,status:`${label} · ${run.progress_message||statusLine(run)}`});
      if(run.status==='completed')return finalResultFromRun(run);
      if(run.status==='failed')throw Error(run.error||run.progress_message||'Supabase run failed.');
      await sleep(750);
    }
  ...
  async function generate(e){
    ...
    try{
      ...
      setResult(final);setProgress({done:final.reviews?.length||total,total,status:(final.purgedReviewCount||0)?`Complete · ${final.purgedReviewCount} purged`:'Complete'});
  ...
  async function generateStore(){
    ...
    const perSku=+f.reviewCount,target=+f.targetAverage;if(!Number.isInteger(perSku)||perSku<5||perSku>250){setErr('Fixture count must be 5–250.');return}
    setGenBusy(true);setErr('');setResult(null);setBulkResult(null);setActiveRun(null);
    const total=selected.length*perSku;
    try{
      await ensureSupabase();
      let finished=0;
      const grouped=await runPool(selected,Math.min(2,concurrency),async(p,pi)=>{
        ...
        const final=await processServerRun(run.id,`${pi+1}/${selected.length} · ${p.productTitle}`,perSku,{base:finished,grandTotal:total});
  ...
  const enabled=products.filter(x=>x.enabled).length,allOn=products.length>0&&enabled===products.length,callEstimate=generationCallEstimate(f.reviewCount),generationSkuCount=mode==='store'?products.filter(x=>x.enabled&&x.status==='done').length:1;
