...
export async function discoverHighVolumeAmazon(req,{referenceSet,productTitle='',productDescription=''}={}){
  const token=gatewayToken(req);if(!token)return{candidates:[],diagnostics:{error:'no_ai_gateway_credential'}};
  ...
  const prompt=`You are resolving an already visually-verified ecommerce product to its best AMAZON.COM listing for INTERNAL QA MODELING. The product may be sold under different private-label brands. SAME PHYSICAL PRODUCT is mandatory; review volume is only a tie-breaker after identity is established.
...
VERIFIED LENS SOURCE CLUES: ${JSON.stringify(clues)}
...
Return at most 6 candidates, sorted first by SAME-PRODUCT confidence and then by public rating/review volume.`;
  let r,raw,j;try{r=await fetch(GW,{method:'POST',headers:{'content-type':'application/json',authorization:`Bearer ${token}`},body:JSON.stringify({model:MODEL,reasoning:{effort:'medium'},input:prompt,tools:[{type:'web_search',search_context_size:'high'}],tool_choice:'required',include:['web_search_call.action.sources','web_search_call.results'],store:false}),signal:AbortSignal.timeout(75000)});raw=await r.text();try{j=JSON.parse(raw)}catch{j=null};if(!r.ok)throw Error(j?.error?.message||j?.message||raw.slice(0,400)||`AI Gateway HTTP ${r.status}`)}catch(e){return{candidates:[],diagnostics:{error:clean(e?.message||e)}}}
  let parsed={};try{parsed=parseObject(extractText(j))}catch(e){return{candidates:[],diagnostics:{error:`parse:${clean(e?.message||e)}`,output:extractText(j).slice(0,1000)}}}
  ...
  return{queries:Array.isArray(parsed.queries)?parsed.queries.slice(0,20):[],candidates,clues,diagnostics:{webSearchCalls:(j?.output||[]).filter(x=>x?.type==='web_search_call').length,rawCandidates:Array.isArray(parsed.candidates)?parsed.candidates.length:0,acceptedCandidates:candidates.length}};
}
