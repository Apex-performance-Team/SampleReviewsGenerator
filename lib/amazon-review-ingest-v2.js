...
export async function collectAmazonReviewsV2(source,{maxReviews=200,maxBatches=4,excludeReviewIds=[]}={}){
  ...
  for(let batch=1;batch<=batchLimit&&all.length<requested;batch++){
    ...
    batches.push({batch,ok:true,endpoint:job.endpoint||null,snapshotId:job.snapshotId,requested:batchReviews,rawRows:parsed.rows.length,parsedBodies:parsed.reviews.length,newBodies,newReviewIds:newIds.length,totalBodies:all.length,totalReviewIds:excludeIds.length,aggregateRatingCount:parsed.aggregateRatingCount,aggregateRating:parsed.aggregateRating,rowKeys:parsed.keys,attempts:job.attempts||[]});
