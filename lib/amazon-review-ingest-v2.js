...
export async function collectAmazonReviewsV2(source,{maxReviews=200,maxBatches=4}={}){
  ...
  const requested=Math.max(1,Math.min(Number(maxReviews)||200,250)),batchLimit=Math.max(1,Math.min(Number(maxBatches)||4,8));
  const all=[],excludeIds=[],seenIds=new Set(),batches=[];let pending=false,failed=0,lastError='';
  for(let batch=1;batch<=batchLimit&&all.length<requested;batch++){
    const remaining=requested-all.length;
    const job=await snapshot({url,reviews_to_not_include:[...excludeIds],max_reviews:remaining,variation_specific:false});
    if(!job.ok){pending=Boolean(job.pending);failed=job.pending?0:1;lastError=`bright_data_${job.error}`;batches.push({batch,ok:false,pending:Boolean(job.pending),snapshotId:job.snapshotId||null,error:lastError});break}
    const parsed=parse(job.data),before=all.length,newIds=[];let newBodies=0;
    for(const r of parsed.reviews){
      const id=clean(r.reviewId);if(id&&!seenIds.has(id)){seenIds.add(id);excludeIds.push(id);newIds.push(id)}
      const key=clean(r.body).toLowerCase().replace(/[^a-z0-9]+/g,' ').trim();
      if(!key||all.some(x=>clean(x.body).toLowerCase().replace(/[^a-z0-9]+/g,' ').trim()===key))continue;
      all.push(r);newBodies++;if(all.length>=requested)break;
    }
    batches.push({batch,ok:true,snapshotId:job.snapshotId,rawRows:parsed.rows.length,parsedBodies:parsed.reviews.length,newBodies,newReviewIds:newIds.length,totalBodies:all.length,totalReviewIds:excludeIds.length,rowKeys:parsed.keys});
    if(newBodies===0||newIds.length===0)break;
    if(all.length===before)break;
  }
  const reviews=dedupe(all,requested);
  return{reviews,provider:'bright_data_amazon_reviews_v2',attempted:batches.length,failed,blocked:0,pending,canonicalUrl:url,requested,rawRows:batches.reduce((n,b)=>n+(b.rawRows||0),0),parsedBodies:batches.reduce((n,b)=>n+(b.parsedBodies||0),0),reviewIdCount:excludeIds.length,reviewIds:excludeIds.slice(0,250),rowKeys:batches.find(b=>b.rowKeys)?.rowKeys||[],batches,elapsedMs:Date.now()-started,error:reviews.length?'':(lastError||'bright_data_amazon_zero_reviews')};
}
