...
export async function collectAmazonReviewsV2(source,{maxReviews=200,maxBatches=4,excludeReviewIds=[]}={}){
  ...
  const all=[],excludeIds=[...initialExcludeIds],seenIds=new Set(initialExcludeIds),seenBodies=new Set(),batches=[];let pending=false,failed=0,lastError='',aggregateRatingCount=null,aggregateRating=null,noCredits=false,httpStatus=null;
  for(let batch=1;batch<=batchLimit&&all.length<requested;batch++){
    ...
    const variants=[
      {inputMode:'dp_full',input:{url,reviews_to_not_include:[...excludeIds],max_reviews:batchReviews,variation_specific:false}},
      {inputMode:'dp_minimal',input:{url,reviews_to_not_include:[...excludeIds]}},
      {inputMode:'reviews_full',input:{url:`https://www.amazon.com/product-reviews/${asin}`,reviews_to_not_include:[...excludeIds],max_reviews:batchReviews,variation_specific:false}}
    ];
    let job=null;
