...
export async function collectAmazonReviewsV2(source,{maxReviews=200,maxBatches=4}={}){
  ...
  const all=[],excludeIds=[],seenIds=new Set(),seenBodies=new Set(),batches=[];let pending=false,failed=0,lastError='',aggregateRatingCount=null,aggregateRating=null;
  for(let batch=1;batch<=batchLimit&&all.length<requested;batch++){
    ...
    batches.push({batch,ok:true,snapshotId:job.snapshotId,rawRows:parsed.rows.length,parsedBodies:parsed.reviews.length,newBodies,newReviewIds:newIds.length,totalBodies:all.length,totalReviewIds:excludeIds.length,aggregateRatingCount:parsed.aggregateRatingCount,aggregateRating:parsed.aggregateRating,rowKeys:parsed.keys});
