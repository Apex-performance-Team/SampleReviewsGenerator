import{classifyBrightDataFailure,brightDataError}from'./bright-data-status.js';
...
function reviewId(x){
  const direct=clean(x?.review_id||x?.reviewId||x?.review_identifier||x?.reviewIdentifier||x?.reviewid||x?.['review-id']||x?.id||'');
  if(direct)return direct;
  for(const v of [x?.review_url,x?.reviewUrl,x?.review_link,x?.reviewLink,x?.url_review,x?.permalink]){const m=String(v||'').match(/(?:customer-reviews\/|\/review\/)(R[A-Z0-9]+)/i);if(m)return m[1].toUpperCase()}
  return'';
}
...
export async function collectAmazonReviewsV2(source,{maxReviews=200,maxBatches=4,excludeReviewIds=[],excludeReviewBodies=[],productTitle='',snapshotFn=snapshot}={}){
  ...
  const requested=Math.max(1,Math.min(Number(maxReviews)||200,250)),batchLimit=Math.max(1,Math.min(Number(maxBatches)||4,8)),perBatchReviews=Math.min(200,requested);
  const initialExcludeIds=[...new Set((Array.isArray(excludeReviewIds)?excludeReviewIds:[]).map(clean).filter(Boolean))];
  const initialBodyKeys=new Set((Array.isArray(excludeReviewBodies)?excludeReviewBodies:[]).map(bodyKey).filter(Boolean));
  const all=[],excludeIds=[...initialExcludeIds],seenIds=new Set(initialExcludeIds),seenProviderBodies=new Set(initialBodyKeys),seenAcceptedBodies=new Set(initialBodyKeys),batches=[],titleFilterReasonCounts={},titleRejectedIds=[],lanes=[{name:'exact_variant',variationSpecific:true},{name:'listing_family',variationSpecific:false}];let laneIndex=0,titleFilteredOut=0,pending=false,failed=0,lastError='',aggregateRatingCount=null,aggregateRating=null,noCredits=false,httpStatus=null;
  for(let batch=1;batch<=batchLimit&&all.length<requested;batch++){
    ...
    const variants=[
      {inputMode:'dp_full',input:{url,reviews_to_not_include:[...excludeIds],max_reviews:batchReviews,variation_specific:lane.variationSpecific}},
      {inputMode:'dp_minimal',input:{url,reviews_to_not_include:[...excludeIds],variation_specific:lane.variationSpecific}},
      {inputMode:'reviews_full',input:{url:`https://www.amazon.com/product-reviews/${asin}`,reviews_to_not_include:[...excludeIds],max_reviews:batchReviews,variation_specific:lane.variationSpecific}}
    ];
    let job=null;
    for(const variant of variants){
      job=await snapshotFn(variant.input);
      job.inputMode=variant.inputMode;
      if(job.ok||job.pending||!customerInactiveLike(job.error))break;
    }
    if(!job.ok){pending=Boolean(job.pending);noCredits=Boolean(job.noCredits);httpStatus=job.httpStatus||null;failed=job.pending?0:1;lastError=job.error?.includes('bright_data_no_credits')?job.error:`bright_data_${job.error}`;batches.push({batch,lane:lane.name,variationSpecific:lane.variationSpecific,ok:false,pending:Boolean(job.pending),noCredits,snapshotId:job.snapshotId||null,inputMode:job.inputMode||null,error:lastError});break}
    const parsed=parse(job.data),newIds=[];let newBodies=0,newProviderBodies=0,parsedReviewIds=0;
    ...
    for(const r of parsed.reviews){
      const id=clean(r.reviewId);if(id)parsedReviewIds++;if(id&&!seenIds.has(id)){seenIds.add(id);excludeIds.push(id);newIds.push(id)}
      const key=bodyKey(r.body),providerBodyIsNew=Boolean(key&&!seenProviderBodies.has(key));if(providerBodyIsNew){seenProviderBodies.add(key);newProviderBodies++}
      const assessment=assessReferenceTitleCompatibility(productTitle,{...r,sourceListingTitle:source?.title});if(!assessment.accepted){if(providerBodyIsNew){titleFilteredOut++;titleFilterReasonCounts[assessment.reason]=(titleFilterReasonCounts[assessment.reason]||0)+1;if(id&&titleRejectedIds.length<50)titleRejectedIds.push(id)}continue}
      if(!key||seenAcceptedBodies.has(key))continue;seenAcceptedBodies.add(key);all.push(r);newBodies++;if(all.length>=requested)break;
    }
    batches.push({batch,lane:lane.name,variationSpecific:lane.variationSpecific,ok:true,endpoint:job.endpoint||null,inputMode:job.inputMode||null,snapshotId:job.snapshotId,requested:batchReviews,rawRows:parsed.rows.length,parsedBodies:parsed.reviews.length,parsedReviewIds,reviewIdCoverage:parsed.reviews.length?parsedReviewIds/parsed.reviews.length:0,newProviderBodies,newBodies,newReviewIds:newIds.length,titleFilteredOut,totalBodies:all.length,totalReviewIds:excludeIds.length,aggregateRatingCount:parsed.aggregateRatingCount,aggregateRating:parsed.aggregateRating,rowKeys:parsed.keys,attempts:job.attempts||[]});
    if(all.length>=requested)break;
    const cannotPageReliably=newIds.length===0;
    const laneExhausted=newProviderBodies===0||cannotPageReliably;
    if(laneExhausted&&laneIndex<lanes.length-1){laneIndex++;continue}
    if(laneExhausted)break;
  }
  const reviews=dedupe(all,requested);
  return{reviews,provider:'bright_data_amazon_reviews_v2',attempted:batches.length,failed,blocked:0,pending,noCredits,httpStatus,canonicalUrl:url,requested,perBatchReviews,initialExcludedReviewIds:initialExcludeIds.length,initialExcludedReviewBodies:initialBodyKeys.size,rawRows:batches.reduce((n,b)=>n+(b.rawRows||0),0),parsedBodies:batches.reduce((n,b)=>n+(b.parsedBodies||0),0),parsedReviewIds:batches.reduce((n,b)=>n+(b.parsedReviewIds||0),0),reviewIdCount:excludeIds.length,reviewIds:excludeIds.slice(0,250),rowKeys:batches.find(b=>b.rowKeys)?.rowKeys||[],aggregateRatingCount,aggregateRating,titleCompatibilityFilter:{version:'shopify-product-title-v1',productTitle:clean(productTitle),inputCount:reviews.length+titleFilteredOut,acceptedCount:reviews.length,rejectedCount:titleFilteredOut,reasonCounts:titleFilterReasonCounts,rejectedReferenceIds:titleRejectedIds},batches,endpoint:batches.find(b=>b.endpoint)?.endpoint||null,elapsedMs:Date.now()-started,error:reviews.length?'':(lastError||'bright_data_amazon_zero_reviews')};
}
