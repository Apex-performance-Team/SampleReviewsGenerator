...
export function classifyBrightDataFailure({status=null,body='',headers=null,service='api'}={}){
  ...
  if(Number(status)===403&&/lacks? the required permissions|permission|forbidden|not authorized/.test(hay))return{kind:'permission',code:'bright_data_balance_permission_required',retryable:false,message:'Balance permission required. Update the Bright Data API token permissions, then refresh.',status:403,raw:text.slice(0,320)};
...
export async function getBrightDataBalance(key,{force=false}={}){
  ...
  if(!token)return{configured:false,ok:false,error:'api_key_missing',balance:null,pendingBalance:null,noCredits:false,permissionRequired:false};
  const prefix=token.slice(0,8);
  if(!force&&cache.value&&cache.keyPrefix===prefix&&Date.now()-cache.at<30000)return cache.value;
  let r,raw;
  try{r=await fetch(`${API}/customer/balance`,{headers:{authorization:`Bearer ${token}`,'accept':'application/json'},cache:'no-store',signal:AbortSignal.timeout(10000)});raw=await r.text()}catch(e){const value={configured:true,ok:null,error:`balance_fetch:${cleanBright(e?.message||e)}`,balance:null,pendingBalance:null,noCredits:false,permissionRequired:false};cache.at=Date.now();cache.keyPrefix=prefix;cache.value=value;return value}
  const failure=classifyBrightDataFailure({status:r.status,body:raw,headers:r.headers,service:'account'});
  if(!r.ok){const permissionRequired=failure?.code==='bright_data_balance_permission_required';const value={configured:true,ok:false,error:failure?.message||`balance_http_${r.status}:${cleanBright(raw).slice(0,180)}`,errorCode:failure?.code||null,balance:null,pendingBalance:null,noCredits:failure?.code==='bright_data_no_credits',permissionRequired,failure};cache.at=Date.now();cache.keyPrefix=prefix;cache.value=value;return value}
  let j;try{j=JSON.parse(raw)}catch{j={}}
  const balance=finiteNumber(j?.balance),pendingBalance=finiteNumber(j?.pending_balance),noCredits=balance!==null&&balance<=0;
  const value={configured:true,ok:true,error:null,errorCode:null,balance,pendingBalance,noCredits,permissionRequired:false,failure:noCredits?{kind:'no_credits',code:'bright_data_no_credits',retryable:false,message:'Bright Data balance is empty or negative. Add funds in Bright Data, then rescan.',status:200,raw:''}:null};
