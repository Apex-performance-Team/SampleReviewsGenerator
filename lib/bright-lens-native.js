...
function organicCount(j){return(Array.isArray(j?.organic)?j.organic.length:0)+(Array.isArray(j?.navigation)?j.navigation.length:0)}
function lensResultCount(j,tab){const organic=organicCount(j);if(tab==='exact_matches')return(Array.isArray(j?.exact_matches)?j.exact_matches.length:0)+organic;if(tab==='products')return(Array.isArray(j?.products)?j.products.length:0)+(clean(j?.general?.type)==='products'&&Array.isArray(j?.images)?j.images.length:0)+organic;if(tab==='visual_matches')return(Array.isArray(j?.visual_matches)?j.visual_matches.length:0)+((['visual_matches','all'].includes(clean(j?.general?.type)))&&Array.isArray(j?.images)?j.images.length:0)+organic;return organic}
...
async function downloadImage(imageUrl,referer){
  try{const u=new URL(imageUrl);if(u.protocol==='http:'&&/(^|\.)instabeamtv\.com$/i.test(u.hostname))imageUrl=u.href.replace(/^http:/,'https:')}catch{}
...
function hasTabPayload(j,tab){if(organicCount(j)>0)return true;if(tab==='exact_matches')return Array.isArray(j?.exact_matches);if(tab==='products')return Array.isArray(j?.products)||(clean(j?.general?.type)==='products'&&Array.isArray(j?.images));if(tab==='visual_matches')return Array.isArray(j?.visual_matches)||(['visual_matches','all'].includes(clean(j?.general?.type))&&Array.isArray(j?.images));return false}
...
function originalResponse(r,raw){return new Response(raw,{status:r.status,statusText:r.statusText,headers:{'content-type':r.headers.get('content-type')||'application/json','cache-control':'no-store','x-srl-lens-transport':'url-uploadbyurl'}})}
async function fetchOriginalLens(input,init){const r=await baseFetch(input,init),raw=await r.text(),json=safeJson(raw);return{r,raw,json}}
function usableOriginalLens(json,tab){return Boolean(json&&typeof json==='object'&&lensResultCount(json,tab)>0)}
async function originalLensResponse(input,init,ctx,lens,reason,preloaded=null){
  const out=preloaded||await fetchOriginalLens(input,init),{r,raw,json}=out;
...
export function installBrightLensNativeTransport(){
  ...
  globalThis.fetch=async function(input,init={}){
    ...
    if(requestUrl===LENS_API&&typeof init?.body==='string'){
      ...
      if(lens){const key=bearer(init.headers),zone=clean(body?.zone),ctx=contextStore.getStore()||{};if(!key||!zone)return baseFetch(input,init);let original=null;try{original=await fetchOriginalLens(input,init);const originalCount=lensResultCount(original.json,lens.tab),usable=original.r.ok&&usableOriginalLens(original.json,lens.tab);if(Array.isArray(ctx.diagnostics))ctx.diagnostics.push({image:safeImageLabel(lens.image),tab:lens.tab,primary:'url_uploadbyurl',status:original.r.status,result:shape(original.json),primaryResultCount:originalCount,primaryUsable:usable});if(usable)return originalResponse(original.r,original.raw)}catch(e){if(Array.isArray(ctx.diagnostics))ctx.diagnostics.push({image:safeImageLabel(lens.image),tab:lens.tab,primary:'url_uploadbyurl',error:clean(e?.message||e).slice(0,260)})}try{const upload=await uploadImage(lens.image,key,zone,ctx.referer||''),result=await tabResult(upload,lens.tab),nativeCount=lensResultCount(result,lens.tab);if(Array.isArray(ctx.diagnostics))ctx.diagnostics.push({image:safeImageLabel(lens.image),tab:lens.tab,fallback:'native_file_upload',upload:shape(upload.json),result:shape(result),nativeResultCount:nativeCount});if(nativeCount===0&&original)return originalLensResponse(input,init,ctx,lens,'native_empty_result_after_url_primary',original);return responseFor(result)}catch(e){if(Array.isArray(ctx.diagnostics))ctx.diagnostics.push({image:safeImageLabel(lens.image),tab:lens.tab,fallback:'native_file_upload',error:clean(e?.message||e).slice(0,260)});if(original)return originalLensResponse(input,init,ctx,lens,'native_error_after_url_primary',original);return baseFetch(input,init)}}
    }
