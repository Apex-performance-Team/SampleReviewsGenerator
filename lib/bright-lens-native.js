...
const NATIVE_PROXY='http://brd.superproxy.io:33335';
...
async function tabResult(upload,tab){
  ...
  if(entry?.link){
    ...
    const r=await proxyFetch(u.href,{headers:{'x-brd-session':upload.session,'x-unblock-data-format':'parsed_light'},dispatcher:upload.agent,signal:AbortSignal.timeout(60000)}),raw=await r.text();
    ...
    if(!hasTabPayload(j,tab))throw Error(`bright_data_native_${tab}_missing_payload:type=${clean(j?.general?.type||'unknown')}`);
    return j;
  }
