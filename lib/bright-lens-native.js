...
async function uploadImage(imageUrl,key,zone,referer){
  ...
  const promise=(async()=>{
    ...
    const r=await proxyFetch('https://lens.google.com/v3/upload',{method:'POST',headers:{'x-brd-session':session},body:fd,dispatcher:agent,signal:AbortSignal.timeout(70000)}),raw=await r.text();if(!r.ok)throw Error(`bright_data_native_upload_${r.status}:${clean(raw).slice(0,220)}`);const json=safeJson(raw);if(!json||typeof json!=='object')throw Error(`Bright Data native Lens upload returned non-JSON data: ${clean(raw).slice(0,180)}`);return{json,agent,session};
  })();
  cacheSet(uploadCache,ck,{at:Date.now(),value:promise},24);return promise;
}

function hasTabPayload(j,tab){if(tab==='exact_matches')return Array.isArray(j?.exact_matches);if(tab==='products')return Array.isArray(j?.products)||Array.isArray(j?.images);if(tab==='visual_matches')return Array.isArray(j?.visual_matches)||Array.isArray(j?.images);return false}
function canonicalTabLink(link){const u=new URL(link),vsrid=u.searchParams.get('vsrid');if(vsrid)u.searchParams.set('vsrid',vsrid);u.searchParams.set('brd_json','1');return u}
...
async function tabResult(upload,tab){
  ...
  if(entry?.link){
    const u=canonicalTabLink(entry.link);
    const r=await proxyFetch(u.href,{headers:{'x-brd-session':upload.session},dispatcher:upload.agent,signal:AbortSignal.timeout(60000)}),raw=await r.text();
    ...
    if(tab==='products'&&Array.isArray(j.images)&&!Array.isArray(j.products))return{...j,products:j.images};
    return j;
  }
  if(tab==='visual_matches'&&Array.isArray(upload.json.images))return{...upload.json,visual_matches:upload.json.images};
  if(tab==='products'&&Array.isArray(upload.json.images))return{...upload.json,products:upload.json.images};
...
export function installBrightLensNativeTransport(){
  ...
  globalThis.fetch=async function(input,init={}){
    ...
    if(requestUrl===LENS_API&&typeof init?.body==='string'){
      ...
      if(lens){const key=bearer(init.headers),zone=clean(body?.zone),ctx=contextStore.getStore()||{};if(!key||!zone)return baseFetch(input,init);try{const upload=await uploadImage(lens.image,key,zone,ctx.referer||''),result=await tabResult(upload,lens.tab);if(Array.isArray(ctx.diagnostics))ctx.diagnostics.push({image:safeImageLabel(lens.image),tab:lens.tab,upload:shape(upload.json),result:shape(result)});return responseFor(result)}catch(e){if(Array.isArray(ctx.diagnostics))ctx.diagnostics.push({image:safeImageLabel(lens.image),tab:lens.tab,error:clean(e?.message||e).slice(0,260)});throw e}}
    }
