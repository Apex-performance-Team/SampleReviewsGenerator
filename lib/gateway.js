...
export function gatewayToken(req){return process.env.AI_GATEWAY_API_KEY||req?.headers?.get?.('x-vercel-oidc-token')||process.env.VERCEL_OIDC_TOKEN||''}
...
export function authMode(req){return process.env.AI_GATEWAY_API_KEY?'api-key':req?.headers?.get?.('x-vercel-oidc-token')?'oidc-header':process.env.VERCEL_OIDC_TOKEN?'oidc-env':process.env.OPENAI_API_KEY?'direct-openai':'none'}
