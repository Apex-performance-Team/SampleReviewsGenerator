...
function reviewVariantTitle(review){
  return clean(review?.sourceVariantTitle||review?.variantTitle||review?.variationTitle||'');
}
...
export function assessReferenceTitleCompatibility(productTitle,review={}){
  ...
  const sourceProductTitle=reviewProductTitle(review),sourceVariantTitle=reviewVariantTitle(review),sourceMode=modeFrom(sourceProductTitle),variantMode=modeFrom(sourceVariantTitle),text=reviewText(review),indoor=signalSummary(text,REVIEW_SIGNALS.indoor),outdoor=signalSummary(text,REVIEW_SIGNALS.outdoor);
  let reason='compatible_or_unresolved';
  if(target.outdoor&&!target.indoor){
    if(sourceVariantTitle&&variantMode.indoor&&!variantMode.outdoor)reason='source_variant_title_conflicts_with_outdoor_shopify_title';
    else if(sourceProductTitle&&sourceMode.indoor&&!sourceMode.outdoor)reason='source_product_title_conflicts_with_outdoor_shopify_title';
    ...
    if(sourceVariantTitle&&variantMode.outdoor&&!variantMode.indoor)reason='source_variant_title_conflicts_with_indoor_shopify_title';
    else if(sourceProductTitle&&sourceMode.outdoor&&!sourceMode.indoor)reason='source_product_title_conflicts_with_indoor_shopify_title';
  ...
  if(reason==='compatible_or_unresolved'&&target.omni){
    if(sourceVariantTitle&&variantMode.directional)reason='source_variant_title_conflicts_with_omni_shopify_title';
    else if(sourceVariantTitle&&variantMode.indoor&&!variantMode.omni)reason='source_variant_title_conflicts_with_omni_shopify_title';
    else if(sourceProductTitle&&sourceMode.directional)reason='source_product_title_conflicts_with_omni_shopify_title';
  ...
  return{accepted:reason==='compatible_or_unresolved',reason,productMode:target,reviewMode:sourceMode,variantMode,sourceProductTitle:sourceProductTitle||null,sourceVariantTitle:sourceVariantTitle||null,indoorScore:indoor.score,outdoorScore:outdoor.score,indoorSignals:indoor.signals,outdoorSignals:outdoor.signals};
}
