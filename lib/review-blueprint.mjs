...
const VOICES=[
  'plainspoken shopper','warm casual buyer','dry and understated user','blunt practical owner','careful note-taker',
  'casual and clipped buyer','enthusiastic but specific user','skeptical buyer','budget-conscious buyer','comparison-minded shopper',
  'detail-noticing owner','low-key appreciative user','problem-solving buyer','first-time user','experienced owner',
  'busy-household buyer','small-space buyer','convenience-focused user','performance-focused user','appearance-sensitive buyer',
  'manual-reading buyer','cautiously optimistic user','minimalist reviewer','story-led customer','mixed-feelings buyer',
  'parent or family buyer','renter or apartment user','repeat buyer','gift giver','nontechnical buyer',
];
const STRUCTURES=[
  'starts with the actual use case, then the result','short first impression followed by one practical detail','personal expectation, then what actually happened',
  'small problem, what they tried, and how it ended','one useful detail and a plain closing','what worked first, then a small reservation',
  'small reservation first, then why they kept using it','brief before-and-after without a formal wrap-up','direct answer to a buyer concern',
  'observation, comparison, then a plain takeaway','specific moment, then why it mattered','two short observations in plain language',
  'setup context, regular use, and one simple takeaway','context first, then product mention','plain complaint with the reason right after',
  'quick note written after ordinary use','practical tip with a sentence of context',
];
const TEXTURES=[
  'uses contractions where natural','mostly short sentences with one fragment','measured sentences with no hype',
  'one parenthetical aside at most','slightly informal wording without forced slang','specific nouns and restrained adjectives',
  'one mild hesitation or qualifier','compact wording that leaves some obvious background unsaid','a conversational turn such as but or honestly',
  'one ordinary situational detail when appropriate','slightly uneven rhythm but clear meaning','matter-of-fact wording with no marketing phrases',
  'one comparison to the prior situation, not a competing brand','states a limitation without dramatizing it',
  'personal reaction followed by what they noticed','a practical recommendation with a condition','varied sentence lengths and no stock closing',
  'sounds typed after use, not edited for a brochure','keeps the experience a little messy but understandable','avoids formal review-template transitions',
];
const LENGTHS=[
...
const FALLBACK_THEME_FOCI=[
  ...
  'comparison to more expensive alternative','installation hardware or included parts','single-user or single-device setup','multi-user or multi-device expectations','weather or environment limit',
  'quality control concern','simple satisfaction','balanced positives and negatives','surprise benefit','not for every situation',
  ...
  'plain positive review','plain recommendation','buyer-specific constraint','ordinary convenience','recommendation with a small reservation',
  ...
  return Array.from({length:count},(_,index)=>{
    ...
    return{
      ...
      scenarioVariants:[
        ...
        'customer describes a practical compromise around '+focus,
        'customer compares expectations to the result for '+focus,
        'customer explains the rating through '+focus,
      ],
