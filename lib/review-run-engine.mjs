...
export async function processRun(req,id){
  let run=await getRun(id),origin=originFrom(req);
  if(['completed','canceled'].includes(run.status))return run;
  try{
    const scanned=await ensureScanned(run,origin);run=await getRun(id);
    const updated=run.mode==='source_rewrite'?await processSourceRewrite(run,origin,scanned.input,run.result_json||{}):await processPdp(run,origin,scanned.input,run.result_json||{});
    return updated;
  }catch(error){
    return updateRun(id,{status:'failed',current_step:'failed',progress_message:'Run failed.',error:clean(error.message,1000)});
  }
}
