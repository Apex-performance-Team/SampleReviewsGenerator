...
export function catalogStatus(catalog,children,workflowStatus=null){
  const rows=Array.isArray(children)?children:[],requested=Number(catalog?.requested_count)||rows.reduce((n,x)=>n+(Number(x.requested_count)||0),0),completed=rows.reduce((n,x)=>n+(Number(x.completed_count)||0),0),purged=rows.reduce((n,x)=>n+(Number(x.purged_count)||0),0),completeSkus=rows.filter(x=>x.status==='completed').length,failedSkus=rows.filter(x=>x.status==='failed').length,canceledSkus=rows.filter(x=>x.status==='canceled').length,input=catalog.input_json||{};
  return{catalog:{id:catalog.id,status:catalog.status,mode:catalog.mode,productTitle:catalog.product_title||input.productTitle||'',bulk:Boolean(input.bulk),concurrency:Number(input.concurrency)||1,requestedCount:requested,completedCount:completed,purgedCount:purged,currentStep:catalog.current_step,progressMessage:catalog.progress_message,error:catalog.error,createdAt:catalog.created_at,updatedAt:catalog.updated_at,completedAt:catalog.completed_at||null,workflowRunId:catalog.result_json?.workflowRunId||null,workflowStatus},progress:{done:Math.min(requested,completed),total:requested,percent:requested?Math.floor(Math.min(requested,completed)/requested*100):0,completeSkus,failedSkus,canceledSkus,totalSkus:rows.length},children:rows.map(compactReviewRun)};
}
