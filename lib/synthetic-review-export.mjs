export const SYNTHETIC_REVIEW_HEADERS=[
  ...
  'requested_review_count','target_average','actual_average','rating_distribution','generation_ai_calls_attempted','generation_ai_calls_expected','generation_ai_calls_cap','corpus_qa_status','corpus_diversity_score','corpus_repair_count','theme_count','max_theme_use','rating_compatibility_fallback',
  ...
  'reference_led','reference_id','reference_platform','reference_provider','reference_source_url','reference_rating',
...
function reviewRow({result,review,productTitle,productUrl}){
  const profile=review?.personaProfile||{},blueprint=review?.blueprint||{},qa=result?.corpusDiagnostics||{},plan=result?.planDiagnostics||{},input=result?.input||{},calls=result?.generationCallBudget||{};
  return{
    ...
    requested_review_count:input.reviewCount??result?.reviewCount??'',target_average:input.targetAverage??result?.targetAverage??'',actual_average:result?.actualAverage??'',rating_distribution:result?.distribution||{},generation_ai_calls_attempted:calls.aiCallsAttempted??'',generation_ai_calls_expected:calls.expected??'',generation_ai_calls_cap:calls.capped??'',corpus_qa_status:qa.qaStatus||'',corpus_diversity_score:qa.overallDiversityScore??'',corpus_repair_count:result?.corpusRepairCount??0,theme_count:plan.themeCount??'',max_theme_use:plan.maxThemeUse??'',rating_compatibility_fallback:Boolean(plan.ratingCompatibilityFallback),
    ...
    reference_led:Boolean(review?.referenceLed),reference_id:review?.referenceId||'',reference_platform:review?.referencePlatform||'',reference_provider:review?.referenceProvider||'',reference_source_url:review?.referenceSourceUrl||'',reference_rating:review?.referenceRating??'',
