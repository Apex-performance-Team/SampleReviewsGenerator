export const SYNTHETIC_REVIEW_HEADERS=[
  ...
  'requested_review_count','generated_review_count','final_review_count','purged_review_count','purged_review_ids','target_average','actual_average','rating_distribution','original_rating_distribution','reference_available','reference_led_total','pdp_only_total','generation_ai_calls_attempted','generation_ai_calls_expected','generation_ai_calls_cap','corpus_qa_status','corpus_diversity_score','corpus_repair_count','theme_count','max_theme_use','rating_compatibility_fallback',
  ...
  'plausibility_action','plausibility_flags','style_action','style_flags','diversity_repaired','diversity_repair_reasons',
];
...
function reviewRow({result,review,productTitle,productUrl}){
  ...
  return{
    ...
    requested_review_count:input.reviewCount??result?.requestedReviewCount??result?.reviewCount??'',generated_review_count:result?.generatedReviewCount??input.reviewCount??'',final_review_count:result?.finalReviewCount??result?.reviews?.length??'',purged_review_count:result?.purgedReviewCount??0,purged_review_ids:(result?.purgedReviews||[]).map(review=>review.id),target_average:input.targetAverage??result?.targetAverage??'',actual_average:result?.actualAverage??'',rating_distribution:result?.distribution||{},original_rating_distribution:result?.originalDistribution||result?.distribution||{},reference_available:coverage.available??'',reference_led_total:coverage.referenceLedTotal??plan.referenceLedTotal??'',pdp_only_total:coverage.pdpOnlyTotal??'',generation_ai_calls_attempted:calls.aiCallsAttempted??'',generation_ai_calls_expected:calls.expected??'',generation_ai_calls_cap:calls.capped??'',corpus_qa_status:qa.qaStatus||'',corpus_diversity_score:qa.overallDiversityScore??'',corpus_repair_count:result?.corpusRepairCount??0,theme_count:plan.themeCount??'',max_theme_use:plan.maxThemeUse??'',rating_compatibility_fallback:Boolean(plan.ratingCompatibilityFallback),
    ...
    plausibility_action:review?.plausibilityAction||'',plausibility_flags:review?.plausibilityFlags||[],style_action:review?.styleAction||'',style_flags:review?.styleFlags||[],diversity_repaired:Boolean(review?.diversityRepaired),diversity_repair_reasons:review?.diversityRepairReasons||[],
  };
}
